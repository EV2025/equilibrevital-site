...
for (const path of ['index.html', 'reservation.html', 'inscription-confirmee.html']) {
  ...
  if (path !== 'index.html' && !/pwa-install\.js/.test(html)) errors.push(`${path}: aide à l’installation absente`);
}
if (!/\{label:'Application',href:'application\.html'\}/.test(fs.readFileSync('assets/js/v39-complet-coherence-roles.js', 'utf8'))) errors.push('pied de page: lien discret vers l’application absent');
...
if (!cardioFitProgramme || cardioFitProgramme.audience !== '13 ans et plus') errors.push('assets/data/programmes-v84.json: tranche d’âge Cardio Fit incorrecte');
